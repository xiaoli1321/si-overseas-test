...
watch(query, async () => {
  page.value = 1;
  if (query.value) {
    await store.searchBySnRemote(query.value);
  }
}, { immediate: true });
...
async function openDevice(sn: string) {
  await store.selectDeviceRemote(sn);
  ...
  <div class="page active" id="page-detect-devices">
    <div class="page-body">
      <div class="devices-header slide-up stagger-1">
        ...
        <button class="btn btn-secondary" type="button" @click="goBackToPreviousPage">&#8592; Back to Device Detection</button>
      </div>
