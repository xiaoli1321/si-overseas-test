...
describe('AppShell', () => {
  ...
  it('shows processing sessions before detection records are submitted', async () => {
    ...
    await wrapper.find('button[aria-label="Session manager"]').trigger('click');
    const sessionsBody = () => wrapper.findAll('.drawer-body')[0];
  ...
  it('lets session manager open a verdict and confirm clearing sessions', async () => {
    const router = makeRouter();
    await router.push('/chat');
    await router.isReady();
    const wrapper = mount(AppShell, {
    ...
    await wrapper.find('button[aria-label="Session manager"]').trigger('click');
    expect(wrapper.find('button[data-test="session-open-records"]').exists()).toBe(false);

    await wrapper.find('button[data-test="session-clear"]').trigger('click');
    expect(store.sessions.value).toHaveLength(1);
    expect(wrapper.find('.session-confirm-modal').text()).toContain('Clear all sessions?');

    await wrapper.find('button[data-test="session-clear-confirm"]').trigger('click');
  ...
  it('highlights detect record while viewing a verdict opened from records', async () => {
    ...
    await router.push({
      name: 'detect-record',
      params: { sn: 'P2251212806JND44', recordId: 'REC-1' },
      query: { category: 'Sensor falling off', from: 'records' },
    });
    await flushPromises();
  ...
  it('shows manager-only menu items (dashboard, account center) based on role', async () => {
    const router = makeRouter();
    await router.push('/chat');
    await router.isReady();

    // Manager sees Dashboard + Account center entries.
    const store = useDemoStore();
    ...
    expect(wrapper.find('[data-test="account-center-link"]').exists()).toBe(true);
    // Create account moved to the Account Center page — no longer in the menu.
    expect(wrapper.find('[data-test="account-create-user"]').exists()).toBe(false);
    ...
    await flushPromises();
    expect(wrapper2.find('[data-test="dashboard-link"]').exists()).toBe(false);
    expect(wrapper2.find('[data-test="account-center-link"]').exists()).toBe(false);
  });
  ...
  it('closes the account dropdown when clicking outside', async () => {
    ...
    // Dispatch a click event outside the account-menu-wrap container
